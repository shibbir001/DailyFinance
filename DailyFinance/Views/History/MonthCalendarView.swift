...
struct MonthCalendarView: View {
            ...
            .sheet(isPresented: $showDayView) {
                if let date = selectedDate {
                    CalendarDayView(date: date)
                        ...
                        .onDisappear {
                            // ✅ Deduplicate before reloading
                            // prevents doubling of monthly totals
    ...
    var monthStatsBar: some View {
        ...
        return HStack(spacing: 0) {
            ...
            VStack(spacing: 4) {
                Text("Balance").font(.caption2).foregroundColor(.secondary)
                Text(formatCurrency(summary?.netBalance ?? 0))
                    .font(.subheadline).fontWeight(.bold)
                    .foregroundColor((summary?.isProfit ?? true) ? .green : .red)
            }
            .frame(maxWidth: .infinity)
        }
        .padding()
            ...
            VStack(spacing: 2) {
                ...
                if hasIncome {
                    Text(formatAmount(income))
                        ...
                        .foregroundColor(.green).lineLimit(1).minimumScaleFactor(0.5)
                } else {
                ...
                if hasExpense {
                    Text(formatAmount(expense))
                        .font(.system(size: 8, weight: .medium))
                        .foregroundColor(.red).lineLimit(1).minimumScaleFactor(0.5)
                } else {
                    Text(" ").font(.system(size: 8))
                }
            }
    ...
    var legend: some View {
        HStack(spacing: 12) {
            ...
            Spacer()
            Text("Tap to view").font(.caption2).foregroundColor(.secondary)
        }
    ...
    func loadDailyData() {
        ...
        var components = DateComponents()
        components.year = year; components.month = month; components.day = 1
        let monthKey = formatter.string(from: Calendar.current.date(from: components) ?? Date())

        let allSummaries   = coreData.fetchAllSummaries()
        let monthSummaries = allSummaries.filter { $0.date?.hasPrefix(monthKey) == true }

        for summary in monthSummaries {
            guard let dateStr = summary.date else { continue }
            data[dateStr] = (income: summary.totalIncome, expense: summary.totalExpense)
        }
        ...
        if !data.isEmpty {
            ...
            let transactions = coreData.fetchTransactions(month: month, year: year)
    ...
    // MARK: - Helpers
    // ✅ Always local timezone — matches CoreDataManager.smartUpdateSummary
    func dateString(_ date: Date) -> String {
        let f = DateFormatter()
        f.dateFormat = "yyyy-MM-dd"
        f.timeZone   = TimeZone.current  // ✅ timezone fix
